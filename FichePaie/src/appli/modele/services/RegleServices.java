...
public class RegleServices {
	...
	/**
	 * Verifie que le parametre action est une action valide
	 * <i> Cotisation = taux </i>
	 * @param regle Règle dont l'action est à vérifier
	 ...
	 * @param regle Règle dont la condition est à vérifier
	...
	public static boolean conditionIsValide(String condition)
	{
		...
		if(champs.length%3 == 0)
		{
			...
			{
				Variable variable = daoF.getVariableDAO().getByLibelle(champs[i]);
				if(variable != null)
				{
					if(champs[i+1] == "<" || champs[i+1] == "<=" || champs[i+1] == "=" || champs[i+1] == ">=" || champs[i+1] == ">" || champs[i+1] == "!=")
					{
						return true;
					}
				}
	 ...
	 * @param condition	Condition à découper
